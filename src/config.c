...
/* ---------------------------------------------------------------------------
 * WriteConfigFile — persist the launcher-editable settings.
 *
 * Surgical in-place update: the existing config.ini text is preserved verbatim
 * (comments, key order, the [KeyMap] shortcuts and [GamepadMap] button maps)
 * and only the handful of keys the GUI launcher owns are rewritten. A key that
 * is present (even commented out) is replaced in place; a key missing from its
 * section is appended to the end of that section; a section missing entirely is
 * appended at the end of the file. Each SNES game's main.c calls this after the
 * launcher returns PLAY so the chosen settings survive the next boot.
 * ------------------------------------------------------------------------- */

typedef struct CfgKV {
  const char *section;   /* section name without brackets, e.g. "Graphics" */
  const char *key;       /* canonical key name */
  char        val[600];  /* serialized value */
...
/* Append any not-yet-written keys belonging to `sec` (used when leaving a
 * section that existed but lacked some of our keys). */
...
/* True if `line` assigns `key` (allowing leading spaces and a leading '#', so a
 * commented-out default like "#OutputMethod = SDL" is matched and uncommented). */
...
void WriteConfigFile(const char *filename) {
  ...
  CfgBuf out = { 0 };
  char cur[64] = "";  /* current section name */
  for (char *s = data; s && *s; ) {
    char *eol = strchr(s, '\n');
    ...
    while (*t == ' ' || *t == '\t') t++;
    if (*t == '[') {
      /* leaving the previous section: append any of its keys we never saw */
  ...
  /* Flush the final section, then append whole sections that never appeared. */
  CfgFlushSection(&out, kvs, N, cur);
  for (int i = 0; i < N; i++) {
    if (kvs[i].done)
      continue;
