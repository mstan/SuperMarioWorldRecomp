...
enum { HEADER = 64, ACTOR_HEADER = 8, TRAILER = 4 };
...
size_t coop_machine_save_size(const CoopMachine *m) {
    ...
    return HEADER+core+m->actor_count*(ACTOR_HEADER+COOP_GUEST_BYTES)+TRAILER;
}
bool coop_machine_save(const CoopMachine *m,void *data,size_t capacity) {
    ...
    memcpy(p,"CNR1",4);put32(p+4,1);memcpy(p+8,rom_id,32);
    size_t core=coop_session_save_size(&m->session);
...
bool coop_machine_load(CoopMachine *m,const void *data,size_t size) {
    ...
    if(memcmp(p,"CNR1",4)||u32(p+4)!=1||memcmp(p+8,rom_id,32)||u32(p+48)!=layout_id()||
        u32(p+60)>1||u32(p+56)>0x29||u32(p+size-4)!=crc(p,size-4))return false;
    size_t core=u32(p+40),count=u32(p+44);
    if(core>size-HEADER-TRAILER || count>(size-HEADER-TRAILER-core)/(ACTOR_HEADER+COOP_GUEST_BYTES) ||
        size!=HEADER+core+count*(ACTOR_HEADER+COOP_GUEST_BYTES)+TRAILER)return false;
    CoopMachine tmp={0};
