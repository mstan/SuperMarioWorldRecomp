...
void ChangeWindowScale(int scale_step) {
  ...
  if (snesrecomp_sdl_get_display_usable_bounds(g_window, &bounds)) {
    ...
    if (!snesrecomp_sdl_get_window_borders_size(
            g_window, &bt, &bl, &bb, &br)) {
...
static bool SdlRenderer_Init(SDL_Window *window) {
  ...
  SDL_Renderer *renderer = snesrecomp_sdl_create_renderer(
      g_window, g_config.output_method == kOutputMethod_SDLSoftware,
      g_benchmark_frames == 0);
  if (renderer == NULL) {
    printf("Failed to create renderer: %s\n", SDL_GetError());
    return false;
  }
  if (kDebugFlag) {
    const char *renderer_name = snesrecomp_sdl_renderer_name(renderer);
  ...
  if (g_benchmark_frames > 0) {
    const char *renderer_name = snesrecomp_sdl_renderer_name(renderer);
    fprintf(stderr, "SNESRECOMP_BENCHMARK_RENDERER name=%s vsync=%d\n",
            renderer_name ? renderer_name : "(unknown)",
  ...
  snesrecomp_sdl_set_texture_linear(
      g_texture, g_config.linear_filtering != 0);
...
static void SdlRenderer_GetOutputSize(int *width, int *height) {
  if (!snesrecomp_sdl_get_render_output_size(
...
static void SdlRenderer_BeginDraw(int width, int height, uint8 **pixels, int *pitch) {
  ...
  if (!snesrecomp_sdl_lock_texture(
          g_texture, &g_sdl_renderer_rect, (void **)pixels, pitch)) {
  ...
  if (g_config.enable_audio) {
     ...
     * variable a non-reproducible audio/boot crash report needs. */
    {
      ...
      SDL_AudioDeviceID *devices =
          SDL_GetAudioPlaybackDevices(&ndev);
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_CONTROLLERAXISMOTION:
        ...
        if (gi)
          HandleGamepadAxisInput(
              gi, SNESRECOMP_SDL_EVENT_AXIS(event),
      ...
      case SDL_MOUSEBUTTONDOWN:
        if (event.button.button == SDL_BUTTON_LEFT &&
          ...
          if ((g_win_flags & SDL_WINDOW_FULLSCREEN) == 0 &&
  ...
  for (;;) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      if (event.type == SDL_CONTROLLERDEVICEADDED) {
        ...
        GamepadInfo *gamepad =
            GetGamepadInfo(SNESRECOMP_SDL_EVENT_DEVICE(event));
        if (gamepad) {
        ...
        GamepadInfo *gamepad =
            GetGamepadInfo(SNESRECOMP_SDL_EVENT_AXIS_DEVICE(event));
        if (gamepad)
          HandleGamepadAxisInput(
              gamepad, SNESRECOMP_SDL_EVENT_AXIS(event),
              SNESRECOMP_SDL_EVENT_AXIS_VALUE(event));
      } else if (event.type == SDL_CONTROLLERBUTTONDOWN ||
                 event.type == SDL_CONTROLLERBUTTONUP) {
        GamepadInfo *gamepad =
            GetGamepadInfo(SNESRECOMP_SDL_EVENT_BUTTON_DEVICE(event));
        if (gamepad) {
          int button = RemapSdlButton(
              SNESRECOMP_SDL_EVENT_BUTTON(event));
          if (button >= 0)
